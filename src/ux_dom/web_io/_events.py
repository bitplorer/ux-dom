"""web_io event types/internals. Private; subject to change."""
import inspect
...
class HtmxEvents(BaseEventManager):
    """HTMX activity helpers.

    ``get_events(activity)`` stays a **method** (same as ``BaseEventManager``).
    Activity-specific accessors are ``*_events`` properties (like WebSocketEvents).
    """
    ...
    @property
    def hx_get_events(self):
        return super().get_events(activity="hx-get")

    # Attribute alias (not callable override of get_events)
    get_event_map = hx_get_events
