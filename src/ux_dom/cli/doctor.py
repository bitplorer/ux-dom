...
def run_doctor(
    *,
    ...
    try:
        ...
        tw_ok = True
        tw_detail = "pytailwindcss importable"
    except Exception:
        import shutil

        if shutil.which("tailwindcss"):
            tw_ok = True
            tw_detail = "tailwindcss binary on PATH"
    report.checks.append(
        Check(
            "tailwind",
            tw_ok,
            tw_detail if tw_ok else "optional — pip install pytailwindcss",
