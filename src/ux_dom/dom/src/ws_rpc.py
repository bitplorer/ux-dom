...
"""Optional WebSocket RPC helpers for demos/elements.

Uses document.setUpOrGetWebSocket / document.ux_domMessageHandler from x_element.js.
Prefer higher-level channel integrations for new apps when available.
"""
from ux_dom.dom.htmlelement import HTMLElement

# from ux_dom.dom.src.htmltags import script
...
class ws_rpc(HTMLElement):
    ...
    def render(self, elem_or_str_or_path=None):
        # Signature matches HTMLElement/Component.render; body is fixed JS asset.
        return raw("""
...
(() => {
                ...
                document.ux_domMessageHandler = messageHandler;
...
for (let i = 0; i < elements.length; i++) {
  ...
  let messageHandler = document.ux_domMessageHandler;
  ...
  elements[i].waitForConnection = document.ux_domWaitForConnection[ws_url];
