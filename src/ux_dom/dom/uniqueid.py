...
"""Process-local unique ids for DOM nodes and tests (not crypto-secure)."""
import random
import threading
...
def _uniqueid():
    ...
    old_time = (
        mynow()
    )  # fake init - on very speed machine it could increase your seed to seed + 1... but we have our contingency :)
    # manage seed
...
_uniqueid_gen = _uniqueid()
_uid_lock = threading.Lock()


class _UniqueId:
    """Callable + iterator unique id source.

    * ``uniqueid()`` → new id (preferred)
    * ``next(uniqueid)`` → new id
    """

    def __call__(self):
        with _uid_lock:
            return next(_uniqueid_gen)
