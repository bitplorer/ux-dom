...
"""Parameter/introspection helpers for CLI and diagnostics."""
import inspect
...
def _parameters(func_signature):
    def get_parameters(args: bool = False, kwargs: bool = False):
        ...
        _args = (args_only + args_p, *var_args)  # type: ignore[assignment]

        kw_only_params = (
            tup for tup in _kwargs if tup[1].kind is PARAM_KINDS["KWARGS"][0]
        )
        kw_only_map: dict = Odict(
            (
                (k, v.default)
                if v.default is not inspect.Parameter.empty
                else (
            ...
            for (k, v) in kw_only_params
        )

        var_kwargs: list = [
            (tup[0], {}) for tup in _kwargs if tup[1].kind is PARAM_KINDS["KWARGS"][1]
        ]
        _kwargs = (Odict(**kw_only_map), *var_kwargs)  # type: ignore[assignment]
