...
"""Small functional helpers used across the library. Prefer stdlib in app code."""
import types
import typing
    ...
    return list(
        (
            ...
            if isinstance(item, frozenset)
                ...
                apply_to_set(elem_func, item, obj_func)
                if isinstance(item, set)
                else (
                    apply_to_list(elem_func, item, obj_func)
                    if isinstance(item, list)
                    else (
                        apply_to_tuple(elem_func, item, obj_func)
                        if isinstance(item, tuple)
                        else (
                            apply_to_dict(elem_func, item, obj_func)
    ...
    return dict(
        (
            ...
            if isinstance(item, frozenset)
                ...
                obj_func(apply_to_set(elem_func, item, obj_func), key)
                if isinstance(item, set)
                else (
                    obj_func(apply_to_tuple(elem_func, item, obj_func), key)
                    if isinstance(item, tuple)
                    else (
                        obj_func(apply_to_list(elem_func, item, obj_func), key)
                        if isinstance(item, list)
                        else (
                            obj_func(apply_to_dict(elem_func, item, obj_func), key)
                            if isinstance(item, dict)
