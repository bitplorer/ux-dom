"""Reloader internal: _app. Dev hot-reload support; not a production public API."""
import functools
import logging
import pathlib
import string
...
from ux_dom.web_io import Receive, Scope, Send
from ux_dom.web_io import WebSocketProtocol as WebSocket
...
class HotReloadWebSocketRoute:
    def __init__(
        ...
        self.watchers = [
            FileWatcher(
                path,
                on_change=functools.partial(self._on_changes, on_reload=on_reload),  # type: ignore[arg-type]
            )
