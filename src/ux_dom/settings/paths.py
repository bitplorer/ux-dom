...
class SubdirOrFile(object):
    def __init__(self, sub_directory_or_file: Union[str, List[str], None] = None):
...
class MakePath(SubdirOrFile):
    ...
    def make_path(self):
        ...
        for path in self._paths_to_create:
            ...
            # Treat as file only when basename has a real extension (e.g. app.css).
            # Version dirs like "v1.2.3" must remain directories.
            is_file_like = (
                "." in base
                and not base.startswith(".")
                and len(base.rsplit(".", 1)[-1]) <= 5
                and base.rsplit(".", 1)[-1].isalpha()
            )
            if not is_file_like:
